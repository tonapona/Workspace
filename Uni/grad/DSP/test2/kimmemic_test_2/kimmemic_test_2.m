...
%% Spočítejte vyhlazený odhad vzájemné spektrální výkonové hustoty (CPSD) Welchovou metod -- ASI OK
...
%% MSC - Určete průměrnou koherenci -- ASI BLBE
...
%% CEpstral distance -- BLBE ( vyslo uplne jinak [melo  2.467])???
% Určete zkreslení delšího signálu SA010S01.CSX na bázi kepstrální vzdálenosti a reálného KEPSTRA,

cp=20;
wlen=512 ;
sig1 = loadbin("SA010S01.CSX");
sig2 = loadbin("SA010S01.CS0");
...
%cepstral distance 
cdist = cde(cepr2, cepr1, cp) %%%%%%%%%%%%%%%%%%%%%%%%%nejde????
%(+compute mean )
meancd = mean(cdist)
